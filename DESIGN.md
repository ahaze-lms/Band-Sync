...
| 2026-05-17 | `play.html` setup screen requires per-player track selection, not just instrument + device. Track picker lists the song's tracks (Melody, Bass, Drums…); two players can share a track ("duel mode") for shared scoring with independent stats. Default when player count exceeds track count. |
